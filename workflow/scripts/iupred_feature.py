...
def iupred(fh_in, fh_out):
    ...
    iupred_max = pd.DataFrame(max_dict.items(), columns=['ID', 'iupred_max'])
    ...
    df_iupred = df_iupred[df_iupred.IUPRED2 > 0.5]
    df_iupred['iupred'] = 1
    df_iupred = df_iupred[['ID', 'iupred']].drop_duplicates()
    df_iupred = df_iupred.merge(iupred_max, how='outer').merge(
        iupred_mean, how='outer').merge(iupred_frac, how='outer')
    df_iupred = df_iupred.drop_duplicates()
    df_iupred = df_iupred[['ID', 'iupred_max', 'iupred_mean', 'frac_disordered']]
    df_iupred.to_csv(fh_out, index=False)
    return df_iupred
